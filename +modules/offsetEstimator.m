function [ofdmSignalRXsynchronized,timeOffsetEstimate, frequencyOffsetEstimate] = offsetEstimator(ofdmSignalRX, SNR)
...
% naming of the function according to the lecture notes
    ...
    maxGamma = max(gamma);
    timeOffsetEstimate = find(gamma == maxGamma)-1;
    frequencyOffsetEstimate = -angle(phi(timeOffsetEstimate)) / (2*pi);
    ...
    ofdmSignalRXsynchronized = ofdmSignalRXremovedFrequOffset(timeOffsetEstimate+1:end);
